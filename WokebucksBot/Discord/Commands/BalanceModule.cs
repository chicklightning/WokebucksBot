...
namespace Swamp.WokebucksBot.Discord.Commands
{
    public class BalanceModule : ModuleBase<SocketCommandContext>
    {
		...
		public async Task CheckTransactionsAsync()
		{
			...
			{
				embedBuilder.AddField($"[{transaction.TimeStamp.ToLocalTime}] **{transaction.TransactionInitiator}** {((transaction.Amount > 0) ? "gave" : "took")} ${transaction.Amount}.", $"{transaction.Comment}");
			}
